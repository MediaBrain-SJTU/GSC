...
class VisualSA(nn.Module):
    ...
    def forward(self, local, raw_global):
        ...
        if len(local) == 1 and len(raw_global) == 1:
            l_emb = self.embedding_local[0](local)
            l_emb = self.embedding_local[2](l_emb)
            l_emb = self.embedding_local[3](l_emb)
            g_emb = self.embedding_global[0](raw_global)
            g_emb = self.embedding_global[2](g_emb)
            g_emb = self.embedding_global[3](g_emb)
        else:
...
class EncoderSimilarity(nn.Module):
    ...
    def __init__(self, opt, embed_size, sim_dim, module_name='AVE', sgr_step=3):
    ...
    def forward(self, img_emb, cap_emb, cap_lens):
        ...
        #     cap_ave_i = torch.mean(cap_i, 1)
        #     cap_glo_i = self.t_global_w(cap_i, cap_ave_i)
        #     cap_glo.append(cap_glo_i)
        # cap_glo = torch.cat(cap_glo, 0)

        # for i in range(n_image):
        #     img_i = img_emb[i].unsqueeze(0)
        #     img_i_expand = img_i.repeat(n_image, 1, 1)

        #     img_glo_i = img_glo[i].unsqueeze(0)

        #     sim_loc = torch.pow(torch.sub(img_emb, img_i_expand), 2) + 1e-6
        #     sim_loc = l2norm(self.sim_tranloc_w(sim_loc), dim=-1)

        #     sim_glo = torch.pow(torch.sub(img_glo, img_glo_i), 2) + 1e-6
        #     sim_glo = l2norm(self.sim_tranglo_w(sim_glo), dim=-1)

        #     sim_emb = torch.cat([sim_glo.unsqueeze(1), sim_loc], 1)

        #     Context_cap = SCAN_attention(img_i_expand, cap_emb + 1e-6, smooth=9.0, lens=cap_lens)
        #     sim_loc_cap = torch.pow(torch.sub(Context_cap, img_i_expand), 2)
        #     sim_loc_cap = l2norm(self.sim_tranloc_w(sim_loc_cap), dim=-1)

        #     sim_glo_cap = torch.pow(torch.sub(cap_glo, img_glo_i), 2)
        #     sim_glo_cap = l2norm(self.sim_tranglo_w(sim_glo_cap), dim=-1)

        #     sim_emb_cap = torch.cat([sim_glo_cap.unsqueeze(1), sim_loc_cap], 1)
            
        #     sim_emb = torch.cat([sim_emb, sim_emb_cap], 0)
        ...
        #     # sim_img.append(self.sim_eval_w(sim_vec))
        #     sim_img_cap = self.sim_eval_w(sim_vec)
        #     sim_img.append(sim_img_cap[: n_image])
        #     sim_all.append(sim_img_cap[n_image: ])

        # # (n_image, n_caption)
        # sim_all = torch.cat(sim_all, 1)
        # sim_img = torch.cat(sim_img, 1)

        # for i in range(n_caption):
        #     n_word = cap_lens[i]
        #     cap_i = cap_emb[i, :n_word, :].unsqueeze(0)
        #     cap_i_expand = cap_i.repeat(n_caption, 1, 1)
        #     cap_glo_i = cap_glo[i].unsqueeze(0)

        #     Context_cap = SCAN_attention(cap_i_expand, cap_emb + 1e-6, smooth=9.0, lens=cap_lens)
        #     sim_loc = torch.pow(torch.sub(Context_cap, cap_i_expand), 2)
        #     sim_loc = l2norm(self.sim_tranloc_w(sim_loc), dim=-1)

        #     sim_glo = torch.pow(torch.sub(cap_glo, cap_glo_i), 2) + 1e-6
        #     sim_glo = l2norm(self.sim_tranglo_w(sim_glo), dim=-1)
        ...
        #     sim_cap.append(self.sim_eval_w(sim_vec))

        # sim_cap = torch.cat(sim_cap, 1)

        # return sim_all, sim_img, sim_cap
    
    def forward_topology(self, img_emb, cap_emb, cap_lens):
        sim_img = []
        n_image = img_emb.size(0)
        ...
        img_emb_n = img_emb.unsqueeze(0).repeat(n_image, 1, 1, 1)
        img_emb_ni = img_emb.unsqueeze(1).repeat(1, n_image, 1, 1)
        sim_loc = torch.pow(torch.sub(img_emb_n, img_emb_ni), 2) + 1e-6
        sim_loc = l2norm(self.sim_tranloc_w(sim_loc), dim=-1)

        img_glo_n = img_glo.unsqueeze(0).repeat(n_image, 1, 1)
        img_glo_ni = img_glo.unsqueeze(1).repeat(1, n_image, 1)
        sim_glo = torch.pow(torch.sub(img_glo_n, img_glo_ni), 2) + 1e-6
        sim_glo = l2norm(self.sim_tranglo_w(sim_glo), dim=-1)

        sim_emb = torch.cat([sim_glo.unsqueeze(2), sim_loc], 2)
        sim_emb = sim_emb.view(-1, sim_emb.size(2), sim_emb.size(3))
        if self.module_name == 'SGR':
            for module in self.SGR_module:
                sim_emb = module(sim_emb)
            sim_vec = sim_emb[:, 0, :]
            ...
            sim_vec = self.SAF_module(sim_emb)
        sim_img = self.sim_eval_w(sim_vec)
        sim_img = sim_img.view(n_image, n_image)
        ...
        cap_emb_n = cap_emb.unsqueeze(0).repeat(n_image, 1, 1, 1)
        cap_emb_ni = cap_emb.unsqueeze(1).repeat(1, n_image, 1, 1)
        sim_loc = torch.pow(torch.sub(cap_emb_n, cap_emb_ni), 2) + 1e-6
        sim_loc = l2norm(self.sim_tranloc_w(sim_loc), dim=-1)

        cap_glo_n = cap_glo.unsqueeze(0).repeat(n_image, 1, 1)
        cap_glo_ni = cap_glo.unsqueeze(1).repeat(1, n_image, 1)
        sim_glo = torch.pow(torch.sub(cap_glo_n, cap_glo_ni), 2) + 1e-6
        sim_glo = l2norm(self.sim_tranglo_w(sim_glo), dim=-1)
        ...
        if self.module_name == 'SGR':
            ...
            sim_vec = self.SAF_module(sim_emb)
        sim_cap = self.sim_eval_w(sim_vec)
        sim_cap = sim_cap.view(n_image, n_image)
...
def SCAN_attention(query, context, smooth, eps=1e-8, lens=None):
    """
...
class SGRAF(object):
    ...
    def __init__(self, opt):
        ...
        if 'f30k' in self.opt.data_name:
            self.targets = torch.ones(145000).cuda()
            self.preds = torch.ones(145000).cuda()
            self.preds_targets = torch.ones(145000).cuda()
            self.topos = torch.ones(145000).cuda()
            self.topo_targets = torch.ones(145000).cuda()
        elif 'coco' in self.opt.data_name:
            self.targets = torch.ones(566435).cuda()
            self.preds = torch.ones(566435).cuda()
            self.preds_targets = torch.ones(566435).cuda()
            self.topos = torch.ones(566435).cuda()
            self.topo_targets = torch.ones(566435).cuda()
        elif 'cc152k' in self.opt.data_name:
            self.targets = torch.ones(150000).cuda()
            self.preds = torch.ones(150000).cuda()
            self.preds_targets = torch.ones(150000).cuda()
            self.topos = torch.ones(150000).cuda()
            self.topo_targets = torch.ones(150000).cuda()

    def state_dict(self):
        state_dict = [self.img_enc.state_dict(), self.txt_enc.state_dict(), self.sim_enc.state_dict(), self.targets, self.topos, self.topo_targets]
        return state_dict
    ...
    def load_state_dict(self, state_dict):
        ...
        if len(state_dict) > 3:
            self.targets = state_dict[3]
            self.topos = state_dict[4]
            self.topo_targets = state_dict[5]
    ...
    def forward_sim(self, img_embs, cap_embs, cap_lens, mode='sim'):
        ...
        # raw_sims, raw_sims_img, raw_sims_cap = self.sim_enc(img_embs, cap_embs, cap_lens)
        raw_sims = self.sim_enc(img_embs, cap_embs, cap_lens)
        if mode == 'sim':
            # return torch.sigmoid(raw_sims), raw_sims_img, raw_sims_cap
