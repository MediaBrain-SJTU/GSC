...
class PrecompDataset(data.Dataset):
    ...
    def __init__(self, data_path, data_split, vocab, opt=None):
        ...
        # if use noise index by image: self.noisy_inx = np.arange(img_len)
        self.noisy_inx = np.arange(cap_len) // self.im_div
        if data_split == 'train' and opt.noise_ratio > 0.0:
            ...
            if os.path.exists(noise_file):
                ...
                inx = np.arange(cap_len)
                np.random.shuffle(inx)
                noisy_inx = inx[0: int(noise_ratio * cap_len)]
                shuffle_noisy_inx = np.array(noisy_inx) // 5
        ...
        self.corrs = (self.noisy_inx == (np.arange(self.length) // self.im_div)).astype(int)
    ...
    def __getitem__(self, index):
        ...
        # if use noise index by images: img_id = self.noisy_inx[int(index / self.im_div)]
